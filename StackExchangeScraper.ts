...
// detect: (1) Identifier (2) Question text (which is on the Hyperlink to the question) (3) How long ago the question was asked.
// question/xxxxx/subject is the pattern (xxxxx is the 5 digit question Identifier)
// xx hours ago is the pattern
...
function main() {
  const fragment: string = readLine().trim();
  fragment.replace(/a href="\/questions\/([0-9]+)\/[^>]>([^<])<\/a>(?:(?!relativetime).)relativetime.>((?:(?!span).))<.span>/gsim, (_, id, title, time) => console.log({title};${time}))
}
